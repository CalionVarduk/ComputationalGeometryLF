...
namespace CVStructures
{
	...
	public ref class TreeNode
	{
		public:
			...
			static void goLeft(TreeNode<T>^% OUT_node) { OUT_node = OUT_node->Left; }
			static void goRight(TreeNode<T>^% OUT_node) { OUT_node = OUT_node->Right; }
			static void goUp(TreeNode<T>^% OUT_node) { OUT_node = OUT_node->Parent; }
