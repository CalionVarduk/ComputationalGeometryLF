...
#pragma endregion
	...
	template <class T>
	T BSTree<T>::get(T item)
	{
		TreeNode<T>^ node = _findNode(item);
		if(node != nullptr) return node->Item;
		throw gcnew ArgumentException("Item doesn't exist in the tree.");
	}
	...
	Int32 BSTree<T>::getDepth(T item)
	{
		...
		while(current != nullptr) {
			if(item < current->Item) TreeNode<T>::goLeft(current);
			else if(item > current->Item) TreeNode<T>::goRight(current);
			else return depth;
			++depth;
		}
		return -1;
	...
	bool BSTree<T>::_searchForItemsParent(T item, TreeNode<T>^% OUT_node, bool% OUT_lastSearchMoveRight)
	{
		while(OUT_node != nullptr) {
			if(item < OUT_node->Item) {
				if(OUT_node->HasLeftChild) TreeNode<T>::goLeft(OUT_node);
				else {
				...
				if(OUT_node->HasRightChild) TreeNode<T>::goRight(OUT_node);
				else {
	...
	{
		TreeNode<T>::goLeft(node);
		while(node->HasRightChild) TreeNode<T>::goRight(node);
		return node;
	}
	...
	{
		TreeNode<T>::goRight(node);
		while(node->HasLeftChild) TreeNode<T>::goLeft(node);
		return node;
	}
